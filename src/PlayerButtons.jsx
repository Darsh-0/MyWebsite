...
export default function PlayerButtons({playing, setPlaying, song, setSong, currentTime, setCurrentTime, totalSongs }) {
    const handleNext = () => {
            setSong((prev) => (prev + 1) % totalSongs);
            setCurrentTime(0);
    };

        const handlePrev = () => {
            if (currentTime > 1) {
                setCurrentTime(0); // restart current song
            } else {
                setSong((prev) => (prev - 1 + totalSongs) % totalSongs);
                setCurrentTime(0);
            }
    };
    return (
        <div className="flex flex-row gap-4">
            <div className="flex items-center justify-center w-10 h-10 hover:scale-107 duration-100 cursor-pointer" onClick={handlePrev}>
            ...
            <div className="flex items-center justify-center w-10 h-10 hover:scale-107 duration-100 cursor-pointer" onClick={handleNext}>
                <svg viewBox="0 0 16 16" className="fill-[#B3B3B3] m-1" width={20} height={20}>
